/*
  # Add Student-Admin Assignment Feature

  1. Schema Changes
    - Add `assigned_admin_id` column to users table for students
    - Add foreign key constraint to ensure valid admin assignment
    - Add index for performance
    - Add RLS policies for admin access control

  2. Security
    - Enable RLS policies for student-admin relationships
    - Ensure admins can only see their assigned students
    - Super admins can see all students and assignments
*/
    ...
    EXISTS (
      ...
      AND (
        ...
        OR (
          ...
          AND current_user.id = assigned_admin_id
        )
      )
    )
  );
