/*
# Super Admin System Setup - Part 1

This migration sets up the complete Super Admin system for LetsGoLearn.

## What this migration creates:
1. Adds 'super_admin' role to user_role enum
2. Creates admin activity logs table
3. Creates system settings table  
4. Creates user sessions table
5. Creates admin permissions table
6. Creates system notifications table
7. Sets up all RLS policies
8. Creates utility functions
9. Inserts default system settings
10. Creates super admin user (using workaround for enum issue)
...
-- Step 20: Create a function to safely create super admin user
-- This function will be called after the enum value is committed
...
-- Step 21: Create a temporary table to trigger user creation in next transaction
CREATE TABLE IF NOT EXISTS _temp_super_admin_setup (
  id serial PRIMARY KEY,
  created_at timestamptz DEFAULT now(),
  status text DEFAULT 'pending'
);

-- Insert a record to indicate setup is needed
INSERT INTO _temp_super_admin_setup (status) VALUES ('needs_super_admin_creation')
