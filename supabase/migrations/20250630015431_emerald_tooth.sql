/*
  ...
  1. New Tables
    - `admin_activity_logs` - Tracks all admin actions and activities
    - `system_settings` - Configurable system parameters and settings
    - `user_sessions` - Active user session monitoring and management
    - `admin_permissions` - Granular permission system for administrators
    - `system_notifications` - System-wide messaging and notifications
  ...
  3. Functions
    - `log_admin_activity()` - Log administrative actions
    - `create_system_notification()` - Create system notifications
...
-- First, add super_admin to user_role enum in a separate transaction block
DO $$
BEGIN
...
-- Create system notifications table (without enum reference initially)
CREATE TABLE IF NOT EXISTS system_notifications (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  title text NOT NULL,
  message text NOT NULL,
  notification_type text DEFAULT 'info',
  target_role text, -- Using text instead of enum initially
...
-- Create super admin user (using text casting to avoid enum issues)
DO $$
BEGIN
...
-- Now that the enum value is committed, we can add constraints if needed
...
-- Add check constraint for target roles
ALTER TABLE system_notifications 
ADD CONSTRAINT check_target_role 
CHECK (target_role IS NULL OR target_role IN ('super_admin', 'admin', 'student'));